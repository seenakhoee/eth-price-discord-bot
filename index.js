...
function getPrices() {
	...
	// API for price data.
	axios.get(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${process.env.COIN_ID}`).then(res => {
		...
		if (res.data && res.data[0].current_price && res.data[0].price_change_percentage_24h) {
			...
			client.guilds.find(guild => guild.id === process.env.SERVER_ID).me.setNickname(`${symbol} $${(currentPrice).toLocaleString().replace(/,/g, ',')}`)
